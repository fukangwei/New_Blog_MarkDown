...
&emsp;&emsp;函数模板的数据类型参数标识符实际上是一个类型形参，在使用函数模板时，要将这个形参实例化为确定的数据类型。将类型形参实例化的参数称为模板实参，用模板实参实例化的函数称为模板函数(简单地说，类实例化生成实例，函数模板实例化生成模板函数)。模板函数的生成就是将函数模板的类型形参实例化的过程。使用中应注意的几个问题：
&emsp;&emsp;1. 函数模板允许使用多个类型参数，但在template定义部分的每个形参前必须有关键字`typename`或`class`：
