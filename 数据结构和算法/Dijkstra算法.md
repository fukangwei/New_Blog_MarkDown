...
<img src="./Dijkstra算法/3.png">

因此`dis[3]`要更新为`60`。这个过程有个专业术语叫做`松弛`，即`v1`顶点到`v4`顶点的路程即`dis[3]`，通过`<v3, v4>`这条边松弛成功。这便是`Dijkstra`算法的主要思想：通过`边`来松弛`v1`顶点到其余各个顶点的路程。
&emsp;&emsp;然后，我们又从除`dis[2]`和`dis[0]`外的其他值中寻找最小值，发现`dis[4]`的值最小。通过之前解释的原理，可以知道`v1`到`v5`的最短距离就是`dis[4]`的值，然后把`v5`加入到集合T中。随后考虑`v5`的出度是否会影响我们的数组`dis`的值，`v5`有两条出度，即`<v5, v4>`和`<v5, v6>`，然后我们发现`v1-v5-v4`的长度为`50`，而`dis[3]`的值为`60`，所以我们要更新`dis[3]`的值。另外，`v1-v5-v6`的长度为`90`，而`dis[5]`为`100`，所以我们需要更新`dis[5]`的值。更新后的`dis`数组如下图：

<img src="./Dijkstra算法/4.png">
...
<img src="./Dijkstra算法/5.png">

使用同样原理，分别确定了`v6`和`v2`的最短路径，最后`dis`的数组的值如下：

<img src="./Dijkstra算法/6.png">
