...
&emsp;&emsp;索引查找的比较次数等于算法中查找索引表的比较次数和查找相应子表的比较次数之和。假定索引表的长度为m，子表长度为s，则索引查找的平均查找长度为“ASL = (1 + m) / 2 + (1 + s) / 2 = 1 + (m + s) / 2”。假定每个子表具有相同的长度，即“s = n / m”，则“ASL = 1 + (m + n/m) / 2”。当“m = n / m”(即“m = √n”，此时s也等于“√n”)，“ASL = 1 + √n”最小，时间复杂度为O(√n)。可见，索引查找的速度快于顺序查找，但低于二分查找。
