...
---
&emsp;&emsp;内部类是指在一个外部类的内部再定义一个类。内部类作为外部类的一个成员，并且依附于外部类而存在的。内部类可为静态，可用`protected`和`private`修饰，而外部类只能使用`public`和缺省的包访问权限。内部类主要有以下几类：`成员内部类`、`局部内部类`、`静态内部类`、`匿名内部类`。<!--more-->
