...
&emsp;&emsp;顺序结构的程序语句只能被执行一次。如果您想要同样的操作执行多次，就需要使用循环结构。`Java`中有三种主要的循环结构：`while`、`do while`和`for`。`Java 5`引入了一种主要用于数组的增强型`for`循环。<!--more-->
...
&emsp;&emsp;虽然所有循环结构都可以用`while`或者`do while`表示，但`Java`提供了`for`循环，使一些循环结构变得更加简单：
