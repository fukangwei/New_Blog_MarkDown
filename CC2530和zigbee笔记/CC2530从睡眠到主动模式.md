...
---
&emsp;&emsp;从`PM1`、`PM2`到切换到`PM0`，可以使用睡眠定时器，因为睡眠定时器计时时间到后，`MCU`会产生睡眠定时器中断，电源模式会自动从`PM1/PM2`切换到`PM0`。在`PM3`下，睡眠定时器不起作用，因为`32K`的晶振都关闭了，故从`PM3`切换到`PM0`用的是外部`IO`中断。
