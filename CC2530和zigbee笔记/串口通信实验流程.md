...
---
&emsp;&emsp;实验内容：协调器建立`ZigBee`无线网络，终端节点自动加入网络，然后终端节点向协调器发送字符串`EndDevice`。协调器接受数据并通过串口把接受到的数据传给`PC`端的串口调试助手。流程如下：
...
void GenericApp_SendTheMessage ( void ) {
 ...
    AF_DataRequest (
        ...
        AF_DISCV_ROUTE,
        AF_DEFAULT_RADIUS );
    /* osal_strlen函数返回字符串实际长度，但发送的数据时，要将字符串的几位字符一起发送，包括“\0”位，所以这里要加1 */
    HalLedBlink ( HAL_LED_1, 0, 50, 500 ); /* LED2闪烁 */
