...
&emsp;&emsp;将`data`段中每个单词的头一个字母改为大写字母：
...
&emsp;&emsp;代码如下(在屏幕的第`5`行第`12`列显示`3`个红底高亮闪烁绿色的`a`)：
...
&emsp;&emsp;将`data`段中的第一个字符串复制到它后面的空间中：
...
将`F000`段中的最后`16`个字符复制到`data`段中：
...
&emsp;&emsp;程序在运行中将`start`处的一条指令复制到`s0`处：
...
统计`data`段中数值大于`8`的字节的个数，用`ax`保存统计结果：
...
统计`data`段中数值小于`8`的字节的个数，用`ax`保存统计结果：
