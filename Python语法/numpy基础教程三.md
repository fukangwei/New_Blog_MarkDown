...
numpy.average
    numpy.average(a, axis=None, weights=None, returned=False): Compute the weighted average along the specified axis. Parameters:
...
    Returns: average, [sum_of_weights] : array_type or double. Return the average along the specified axis. When returned is True, return a tuple with the average as the first element and the sum of the weights as the second element. The return type is Float if a is of integer type, otherwise it is of the same type as a. sum_of_weights is of the same type as average. 
    Raises:
...
numpy.full
    numpy.full(shape, fill_value, dtype=None, order='C'): Return a new array of given shape and type, filled with fill_value. Parameters:
...
    If v is longer than a, the arrays are swapped before computation. Parameters:   
