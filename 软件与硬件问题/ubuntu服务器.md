...
&emsp;&emsp;4. 修改`vsftpd`配置文件`vsftpd.conf`，该文件位于`/etc`目录下，相关的配置如下：
...
&emsp;&emsp;`sftp`是`Secure File Transfer Protocol`的缩写，意思是安全文件传送协议，可以为传输文件提供一种安全的加密方法，与`ftp`有着几乎一样的语法和功能。常用的命令如下：
