...
&emsp;&emsp;原始线程相对于多线程的优势在于原始线程是相当轻量极的，原始线程不需要自己的堆栈。相反，所有的线程都运行在同一个堆栈中，上下文的切换是通过堆栈的回转实现的。这在内存受限的系统中是具有优势的，因为那里一个线程的堆栈可能就占用了很大的一部分内存。原始线程的每条线程只需两字节，而且原始线程是纯用C实现的，不需要任何机器有关的汇编语言代码。
