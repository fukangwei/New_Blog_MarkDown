...
意思就是采集数据的时候使用的是隔行扫描的方式，也就是说采集到的`yuv`写到文件中的方式是用隔行扫描的方式保存。如果不将`Interleaved`复选框勾起来，播放器就不能正常的播放`yuv`文件。`V4L2`视频采集代码如下：
...
&emsp;&emsp;**补充说明**：这是我在计算机上的软件设置：

<img src="./Linux视频采集与编码/1.jpg">