...
&emsp;&emsp;用户常见的数据通信的基本方式有两种：并行通信和串行通信。串行通信是计算机常用的接口，如`RS-232`接口。该标准规定采用一个`DB25`芯引脚连接器或`DB9`芯引脚连接器。芯片内部常具有`UART`控制器，其可工作于`Interrupt`(中断模式)或`DMA`(直接内存访问)模式。<!--more-->
