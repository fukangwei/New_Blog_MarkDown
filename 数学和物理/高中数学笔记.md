...
$$
a^{\cfrac{m}{n}} = \sqrt[n]{a^m}(a > 0, m与n \in N^*且n > 1)
$$

&emsp;&emsp;负分数指数幂：

$$
a^{-\cfrac{m}{n}} = \cfrac{1}{a^{\cfrac{m}{n}}}(a > 0, m与n \in N^*且n > 1)
$$
...
- $\vec{\alpha}^2 = |\vec{\alpha}|^2$，即$|\vec{\alpha}| = \sqrt{\vec{\alpha}^2}$
- $\vec{\alpha} \bot \vec{\beta} \Leftrightarrow \vec{\alpha} \cdot  \vec{\beta} = 0$
...
&emsp;&emsp;若存在三个不共面向量$\vec{a}$、$\vec{b}$、$\vec{c}$，那么对空间任一向量$\vec{p}$，存在唯一有序实数组$\{x, \; y, \; z\}$，使得$\vec{p} = x\vec{a} + y\vec{b} + z\vec{c}$。任意不共面的三个向量都可做为空间的一个基底，向量$\vec{a}$、$\vec{b}$、$\vec{c}$都叫做`基向量`。
...
&emsp;&emsp;设不重合的两个平面$\alpha$和$\beta$的法向量分别是$\vec{u}$和$\vec{v}$，那么二面角$\alpha-l-\beta$的大小为$\theta \; (0 \le \theta \le \pi)$：$|cos \; \theta| = \cfrac{|\vec{u} \cdot \vec{v}|}{|\vec{u}| |\vec{v}|}$。<br>
