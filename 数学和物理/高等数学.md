...
&emsp;&emsp;两个向量之和的性质如下，其中等号在$\vec{a}$与$\vec{a}$同向或反向时成立：
...
&emsp;&emsp;三条坐标轴中的任意两条可以确定一个平面，这样定出的三个平面统称为`坐标面`。$x$轴与$y$轴所确定的坐标面叫做`xOy面`，另两个由$y$轴及$z$轴，和由$z$轴及$x$轴所确定的坐标面，分别叫做`yOz面`及`zOx面`。
