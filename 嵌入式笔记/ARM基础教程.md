...
&emsp;&emsp;`Thumb`状态下的寄存器组织与`ARM`状态下的寄存器组织的关系：
...
<img src="./ARM基础教程/5.png" width=50%>
...
&emsp;&emsp;立即寻址也叫立即数寻址，这是一种特殊的寻址方式，操作数本身就在指令中给出，只要取出指令也就取到了操作数。这个操作数被称为`立即数`，对应的寻址方式也就叫做`立即寻址`。例如以下指令：
...
&emsp;&emsp;基址变址寻址就是将寄存器(该寄存器一般称作`基址寄存器`)的内容与指令中给出的地址偏移量相加，从而得到一个操作数的有效地址：
...
&emsp;&emsp;堆栈是一种数据结构，按先进后出(`First In Last Out`，`FILO`)的方式工作，使用一个称作`堆栈指针`的专用寄存器指示当前的操作位置，堆栈指针总是指向栈顶。
...
&emsp;&emsp;在`ARM`汇编语言程序里，有一些特殊指令助记符，这些助记符与指令系统的助记符不同，没有相对应的操作码，通常称这些特殊指令助记符为伪指令，它们所完成的操作称为`伪操作`。伪指令在源程序中的作用是为完成汇编程序作各种准备工作的，这些伪指令仅在汇编过程中起作用，一旦汇编结束，伪指令的使命就完成(类似于`C`语言中的宏定义、关键字等)。在`ARM`的汇编程序中，有如下几种伪指令：符号定义伪指令、数据定义伪指令、汇编控制伪指令、宏指令以及其他伪指令。
