...
其中，`xt`代表时间步`t`的输入；`st`代表时间步`t`的隐藏状态，可看作该网络的`记忆`；`ot`作为时间步`t`时刻的输出；`U`、`V`、`W`是所有时间步共享的参数，共享的重要性在于我们的模型在每一时间步以不同的输入执行相同的任务。当把`RNN`展开的时候，网络可被看作每一个时间步都受上一时间步输出影响(时间步之间存在连接)的前馈网络。
