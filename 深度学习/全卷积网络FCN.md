...
&emsp;&emsp;如下图所示，`FCN`将传统`CNN`中的全连接层转化成卷积层，对应`CNN`网络`FCN`把最后三层全连接层转换成为三层卷积层。`FCN`将这`3`层表示为卷积层，卷积核的大小`(通道数, 宽, 高)`分别为`(4096, 1, 1)`、`(4096, 1, 1)`和`(1000, 1, 1)`。看上去数字上并没有什么差别，但是卷积跟全连接是不一样的概念和计算过程，使用的是之前`CNN`已经训练好的权值和偏置，但是不一样的在于权值和偏置是有自己的范围，属于自己的一个卷积核。因此`FCN`网络中所有的层都是卷积层，故称为全卷积网络。
